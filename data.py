	...
	def loadChunk(self,opt,loadRange=None):
		...
		for c in range(chunkSize):
			...
			if self.loadFixedOut:
				...
				# mask_fixed is the array have 2 value 0 or 1 where 1 at the positon that depth_fixed have exact 0 value and the other is 0
				mask_fixed = depth_fixed!=0
				# Need debug for more information
				depth_fixed[~mask_fixed] = opt.renderDepth
				# store data
				# Here need 2 type of data: depth and mask, the question: mask ????
