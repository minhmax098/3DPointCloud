...
# build graph
# Đọc cái ni nè, nó build graph xong cái ở phía dưới nó tạo session để feed dữ liệu vô cái graph rồi chạy
# Để buil nó dùng cái script rời nó viết riêng trc nữa là cái graph.py, đọc cái đó luôn
with tf.device("/gpu:0"):
	...
	# Mấy cái khúc ở trên thì mi tìm hiểu cho ta 2 cái: 
	# 1. XY là gì, shape của nó như thế nào ? 
	# 2. XYGT là gì, shape của nó như thế nào ? 


	# Cái l1 loss là ri ||a - b||_1
	# Còn cái masked_l1_loss thì ta chưa thấy trong bài báo. (Nhưng nó loanh quanh chỗ ni, đọc đi man. Hiểu rồi chỉ ta)
	loss_XYZ = graph.l1_loss(XY-XYGT)/opt.batchSize
	loss_XYZ += graph.masked_l1_loss(depth-depthGT,maskLogit>0)/opt.batchSize
	
	# Đây là cái công thức cross entropy loss, trong bài báo nó có ghi đó man
	# Tại sao dùng cái này, dùng cái này trong trường hợp nào. Hôm qua ta nói mi làm chưa ??? 
	loss_mask = graph.cross_entropy_loss(maskLogit,maskGT)/opt.batchSize

	# Cái công thức tính loss đây nè, mi vô bài báo coi thử, mở ta coi
	# Tại sao phải kết hợp 2 cái loss nhỏ lại như thế này, đọc báo để tìm câu trả lời. 
...
with tf.Session(config=tfConfig) as sess:
	...
	if opt.fromIt!=0:
		...
		# Need to load the learning rate from the ckpt model here !!
		# Ex: 
		# op.lr = saver.__get_learning_rate  # peseudo code 
		# Currently it's not a problem, cus 20000 iter, the model update the lr 
	...
	for c in range(chunkResumeN,chunkMaxN):
		...
		for i in range(c*opt.itPerChunk,(c+1)*opt.itPerChunk):
			...
			# Cái đầu ra nó là ri: 
			# _: optim, cái ni ko đổi và nó ko dùng tới
			# l: loss mới
			# lx: loss_XYZ mới 
			# lm: Loss_mask mới 
			# ml: maskLogit mới
			# Tóm lại cái đầu ra sẽ là cái cập nhật mới của cái run list cũ
			# Hình như sau khi chạy cái sess run thì các biến khi đc khai báo ở phân graph (khúc 26 tới khúc 66) nó sẽ được cập nhật lại luôn 
			# Tóm lại: runlist mới = sess.run(runlist cũ)
			# Kiếm chứng lại nè: 
			# Run list gồm biến: 
			# optim: line 54
			# loss: line 51
...
# At iteration 600, lost is 29047, if load correctly, the lost is arround this value
